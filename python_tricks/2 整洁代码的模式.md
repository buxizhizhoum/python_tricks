...
如果您听说过一些经验丰富的Python使用者谈论Python或观看过一些会议演讲，
您可能已经听说过dunder一词。如果您想知道这是什么，那么，答案在这里：
...
如果您正在解释器的会话中工作并且想访问先前计算的结果，这会很方便：
...
如果您正在动态地构建对象并且想要在不给他们分配名称的情况下与其进行交互，这也很方便：
...
这是可让您轻松完成简单位置格式化的捷径。  ## TODO: 润色一下
如果您曾经使用过C语言中的printf函数，您会立即意识到它是如何工作的。
这是一个简单的示例：
...
如果您想在单个字符串中进行多个替换，“老式”字符串格式语法会稍有变化。
因为％操作符只需要一个参数，您需要将右侧的参数包裹在一个元组中，像下面这样：

    `>>> 'Hey %s, there is a 0x%x error!' % (name, errno)
     'Hey Bob, there is a 0xbadc0ffee error!'`
     
如果您将一个字典传递给％操作符，您还可以在字符串格式化中使用变量名指定变量：
...
这会使字符串格式化更易于维护和修改。
您不用担心传递的值顺序与字符串格式化中引用顺序的匹配问题。
当然，缺点是需要打更多的字。

我敢肯定您一直想知道为什么这种printf风格的格式化被打上“老式”的标签。 
...
您可以使用format()函数进行简单的基于位置的格式化，和你使用“老式”格式化很像：

    `>>> 'Hello, {}'.format(name)
    'Hello, Bob'`

您在按名称进行变量替换时，可以按照任何你喜欢的顺序进行。
...
但是，从Python 3.6开始，还有一中更好的字符串格式化方法。在下一节中，我将告诉您所有的相关信息。

### #3 文字字符串内插(Python 3.6+)  # TODO: 润色

Python 3.6添加了另一种格式化字符串的方式，称为格式化的文字字符串。
这种格式化字符串的新方法使您可以在字符串常量中嵌入Python表达式。
这是一个简单的示例，可以让您对该功能有所了解：

    `>>> f'Hello, {name}!'
    'Hello, Bob!'`

这种新的格式语法功能强大。 由于您可以嵌入任意Python表达式，因此甚至可以进行运算，
像这样：
...
您可以在官方Python文档中了解有关格式化字符串的更多信息。
...
效果很好，您可能想知道应该在何时使用模板字符串。
我认为，模板字符串最好的使用场景是处理用户生成的格式字符串。
由于降低了复杂度，模板字符串是一个更安全的选择

其他更复杂的字符串格式化技术的可能会在您的程序中引入安全漏洞。
...
吓死了，呵呵！模板字符串阻断了该攻击方式，这使得如果您要将用户输入进行字符格式化，字符串模板是一个更安全的选择：
